...
public class ExecutorTestHelper {
    ...
    public static MappedStatement prepareInsertAuthorMappedStatementWithAutoKey(final Configuration config) {
        ...
        MappedStatement ms = new MappedStatement.Builder(config, "insertAuthor", new StaticSqlSource(config,
                ...
                new ParameterMap.Builder(config, "defaultParameterMap", Author.class,
                        ...
                        }).build()).cache(authorCache).keyGenerator(Jdbc3KeyGenerator.INSTANCE).keyProperty("id")
                .build();
        return ms;
    }
    ...
    public static MappedStatement prepareUpdateAuthorMappedStatement(final Configuration config) {
        final TypeHandlerRegistry registry = config.getTypeHandlerRegistry();
        MappedStatement ms = new MappedStatement.Builder(config, "updateAuthor", new StaticSqlSource(config,
                "UPDATE author SET username = ?, password = ?, email = ?, bio = ? WHERE id = ?"), SqlCommandType.UPDATE)
                ...
                .resultMaps(new ArrayList<ResultMap>() {
                    {
                        add(new ResultMap.Builder(config, "defaultResultMap", HashMap.class,
                                ...
                                new ResultMapping.Builder(config, "section", "section",
                                        registry.getTypeHandler(String.class)).build(), new HashMap<String, String>() {
                            {
    ...
    public static MappedStatement createSelectAuthorWithIDof99MappedStatement(final Configuration config) {
        ...
        MappedStatement ms = new MappedStatement.Builder(config, "selectAuthor",
                ...
                .statementType(StatementType.STATEMENT).parameterMap(
                        new ParameterMap.Builder(config, "defaultParameterMap", Author.class,
                                new ArrayList<ParameterMapping>()).build()).resultMaps(new ArrayList<ResultMap>() {
                    {
                        add(new ResultMap.Builder(config, "defaultResultMap", Author.class,
                                new ArrayList<ResultMapping>() {
                                    {
    ...
    /**
     * 准备复杂的select blog 映射完成的声明
     * // TODO: 2021/6/30 CallYeDeGuo 这个可以很清楚的了解一个 MappedStatement要构造哪些东西
     *
     * @param config
     * @return
     */
    ...
    /**
     * 准备insert author 的已映射声明同时会在之前主动创建key
     *
     * @param config
     * @return
     */
    public static MappedStatement prepareInsertAuthorMappedStatementWithBeforeAutoKey(final Configuration config) {
        ...
        MappedStatement kms = new MappedStatement.Builder(config, "insertAuthor!selectKey",
                new StaticSqlSource(config, "select 123456 from dual"), SqlCommandType.SELECT).keyProperty("id")
                .resultMaps(new ArrayList<ResultMap>() {
                    {
                        add(rm);
                    }
        ...
        MappedStatement ms = new MappedStatement.Builder(config, "insertAuthor", new DynamicSqlSource(config,
                new TextSqlNode(
                ...
                new ParameterMap.Builder(config, "defaultParameterMap", Author.class,
                        ...
                        }).build()).cache(authorCache).keyGenerator(new SelectKeyGenerator(kms, true)).keyProperty("id")
                .build();
        return ms;
    }
